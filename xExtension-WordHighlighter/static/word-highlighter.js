...
/* WordHighlighter v0.0.1 (FreshRSS Extension) */

const HIGHLIGHT_OPTIONS = {
	// context - where to find words to highlight
	context: document.querySelector('#stream'),
	// enable highlighting words in the article (slower❗️)
	enableInArticle: false,
	// whether console logs are enabled
	enableLogs: true,
};
...
//#endregion /* eslint-enable */

function wordHighlighterWrapper(c /* console */, Mark) {
	const markConf = {
		caseSensitive: false,
		...
		exclude: [
			'mark',
			...(HIGHLIGHT_OPTIONS.enableInArticle ? [] : ['article *']),
		],
	};

	const m = new Mark(HIGHLIGHT_OPTIONS.context);
	const changePageListener = debounce(500, (x) => {
		HIGHLIGHT_OPTIONS.enableLogs && c.group('Highlighter: page change');
		stopObserving();
		highlightWords(m, startObserving);
	});

	const mo = new MutationObserver(changePageListener);
	mo.observe(HIGHLIGHT_OPTIONS.context, { subtree: true, childList: true });
	...
	function highlightWords(m, done) {
		...
		let hCount = 0;
		const markConfig = (done) => ({
			...markConf,
			done: (n) => (hCount += n) && done(),
			noMatch: done,
		});

		new Promise((resolve) =>
			m.mark(window.WordHighlighterConf || [], { ...markConfig(resolve) })
		)
			// TODO: add functionality for secondary highlighting:
			// .then(
			// 	new Promise((resolve) =>
			// 		m.mark(HIGHLIGHT_WORDS_SECONDARY, {
			// 			...markConfig(resolve),
			...
			// )
			.finally(() => {
				if (HIGHLIGHT_OPTIONS.enableLogs) {
					c.log(`Highligher: ${hCount} new highlights added in ${performance.now() - start}ms.`);
					c.groupEnd();
				}
				done();
			});
...
try {
	const areOutsideOfMainPageWithFeeds = !(new URL(window.location)).searchParams.get('c');
	if (areOutsideOfMainPageWithFeeds) {
		wordHighlighterWrapper(console, window.Mark || (Error('mark.js library is not loaded')));
		console.log('WordHighlighter: script loaded.✅');
	} else {
		HIGHLIGHT_OPTIONS.enableLogs && console.log('WordHighlighter: ❗️ paused outside of feed page');
	}
} catch (error) {
